...
bool ParseRegistryPath(const char* path, bool& currentUser, wchar_t* subKey, wchar_t* stringName) {
...
// This is a new function designed to be where this API is going in the future.
bool ReadSetting(const char* path, bool& result) {
#ifdef _WIN32
  bool currentUser;
  wchar_t subKey[256];
  wchar_t stringName[256];

  if (!ParseRegistryPath(path, currentUser, subKey, stringName))
    return false;

  wchar_t out[256];
  if (GetRegistryStringW(subKey, stringName, out, false, currentUser)) {
    result = (_wtoi64(out) != 0);
    return true;
  }

  DWORD dw;
  if (GetRegistryDwordW(subKey, stringName, dw, false, currentUser)) {
    result = (dw != 0);
    return true;
  }
#else
