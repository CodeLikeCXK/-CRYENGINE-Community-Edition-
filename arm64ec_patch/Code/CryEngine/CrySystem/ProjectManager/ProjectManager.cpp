...
bool CProjectManager::ParseProjectFile()
{
		...
		gEnv->pSystem->GetArchiveHost()->LoadJsonBuffer(Serialization::SStruct(m_project), projectFileJson.data(), projectFileJson.size()))
	{
		...
		auto findGameDllPath = [this](const char* platform)
		{
			auto it = m_project.legacyGameDllPaths.find(platform);
			return it != m_project.legacyGameDllPaths.end() && !it->second.empty() ? it : m_project.legacyGameDllPaths.end();
		};

		auto gameDllIt = findGameDllPath("any");

		if (gameDllIt == m_project.legacyGameDllPaths.end())
		{
		#if defined(ARM64EC)
			gameDllIt = findGameDllPath("win_arm64ec");
		#endif
		}

		if (gameDllIt == m_project.legacyGameDllPaths.end())
		{
			gameDllIt = findGameDllPath("win_x64");
		}
		...
		#if defined(ARM64EC)
			// Keep older projects usable when they only contain a win_x64 entry.
			// Prefer the ARM64EC output if the corresponding DLL exists.
			if (gameDllIt->first == "win_x64")
			{
				string arm64ecPath = legacyGameDllPath;
				arm64ecPath.replace("bin/win_x64/", "bin/win_arm64ec/");
				arm64ecPath.replace("bin\\win_x64\\", "bin\\win_arm64ec\\");
				if (gEnv->pCryPak->IsFileExist(arm64ecPath.c_str(), ICryPak::eFileLocation_Any))
					legacyGameDllPath = arm64ecPath;
			}
		#endif
