...
HRESULT CALLBACK CD3D9Renderer::OnD3D11CreateDevice(D3DDevice* pd3dDevice)
{
	...
	if (rd->m_MaxTextureMemory == 0)
	{
		// On UMA / integrated GPU architectures (such as Qualcomm Adreno on Windows ARM64, or Intel/AMD APUs)
		// DedicatedVideoMemory is reported as 0 by DXGI, and video memory is shared from system RAM.
		const SIZE_T sharedMem = rd->m_devInfo.AdapterDesc().SharedSystemMemory;
		if (sharedMem > 0)
		{
			// Allocate up to 3GB from shared memory (or half of shared system memory)
			rd->m_MaxTextureMemory = min(sharedMem / 2, (SIZE_T)3072 * 1024 * 1024);
		}
		else
		{
			rd->m_MaxTextureMemory = 2048 * 1024 * 1024;
		}
	}
#else
