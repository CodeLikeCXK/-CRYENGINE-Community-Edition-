...
{
#if defined(_MSC_VER)
	return (float32x4_t)vdupq_n_u32(0xffffffff);
#else
...
{
#if defined(_MSC_VER)
	return vdupq_n_f32(FLT_EPSILON);
#else
...
ILINE uint32 SignMask(vec4 V)
{
	...
	uint8x8_t idx_sign;
	idx_sign.n64_u64[0] = 0x000000000F0B0703ULL;
	uint8x8_t sign_bytes = vtbl2_u8(tbl, idx_sign);
	return sign_bytes.n64_u32[0];
#else
