...
all_versions = [ ('14.3', '10.0.20348.0'), ('14.0', '10.0.17134.0'), ('14.0', '10.0.15063.0'),('14.0', '10.0.14393.0'), ('14.0', '10.0.10586.0'), ('14.0', '10.0.10240.0'), ('12.0', '8.1'), ('11.0', '8.0')]
...
def get_msvc_for_target_arch(path, target, arch, verbose):
	for subfolder in get_subfolders_for_target_arch(target, arch):
		compiler_bins = [os.path.join(path, 'bin', subfolder)]
		# VS 2017+ stores cross compiler tools below a Host* directory.
		if target == 'arm64':
			compiler_bins += [
				os.path.join(path, 'bin', 'Hostx64', subfolder),
				os.path.join(path, 'bin', 'Hostarm64', subfolder),
				os.path.join(path, 'bin', 'Hostx86', subfolder)
			]
		for compiler_bin in compiler_bins:
			if not (os.path.isdir(compiler_bin) and os.path.isfile(os.path.join(compiler_bin, 'cl.exe'))):
				continue
			# Set up compiler
			result = {}
			result['root'] = path
...
def gather_msvc_versions(verbose, min_version):
	...
	# VS 2017+ is registered through the installer and is not visible through
	# the legacy VisualStudio\14.0 registry key used above. Use vswhere when it
	# is available so ARM64EC can find the modern ARM64 toolset.
	...
	if os.path.isfile(vswhere):
		try:
			...
			path = process.communicate()[0].strip()
			if isinstance(path, bytes):
				path = path.decode('utf-8', 'ignore')
			if process.returncode == 0 and os.path.isdir(path):
				msvc_root = os.path.join(path, 'VC', 'Tools', 'MSVC')
				if os.path.isdir(msvc_root):
					versions = [entry for entry in os.listdir(msvc_root) if os.path.isdir(os.path.join(msvc_root, entry))]
					if versions:
						path = os.path.join(msvc_root, sorted(versions)[-1])
				vc_paths.setdefault('14.3', path)
				if verbose:
					Logs.info('Detected modern Visual Studio ARM64 toolset through vswhere (path: %s)' % path)
		except:
			pass
