#############################################################################
## Crytek Source File
# Copyright 2015 Crytek GmbH / Crytek Group. All rights reserved.
##
## Creator: Nico Moss
## Date: July 03, 2015
...
def execute_module_extension(ctx, extension_name, kw, entry_prefix, platform, configuration):
	if extension_name in MODULE_EXTENSIONS:
		# ARM64EC can consume existing x64 user-mode libraries. Reuse the
		# x64 dependency selection until an ARM64EC vendor package exists.
		extension_platform = 'win_x64' if platform == 'win_arm64ec' else platform
		for extentsion in MODULE_EXTENSIONS[extension_name]:
			extentsion(ctx, kw, entry_prefix, extension_platform, configuration)
	else:
