...
#if defined(ARM64EC)
	// Layout state contains positional Qt indexes and serialized plugin state.
	// It is not portable between the x64 and ARM64EC plugin sets; restoring an
	// old x64 layout can feed invalid indexes into Qt 5.12's QVector checks.
	// Start ARM64EC with the default layout and let subsequent saves create a
	// native-compatible state file.
	(void)state;
	return;
#endif
