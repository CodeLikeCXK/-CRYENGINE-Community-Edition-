...
def get_available_configs():
    return [
        ...
        # Visual Studio 2022 ARM64EC
        {
            'title': 'Visual Studio 2022 ARM64EC',
            'cmake_toolchain': 'toolchain/windows/WindowsPC-ARM64EC-MSVC.cmake',
            'cmake_generator': 'Visual Studio 17 2022',
            'cmake_generator_platform': 'ARM64EC',
            'cmake_builddir': 'solutions/arm64ec',
            'compiler': {
                'reg_key': winreg.HKEY_CLASSES_ROOT,
                'key_path': r'\VisualStudio.DTE.17.0'
            }
...
def get_project_solution_dir(project_path):
    ...
    arm64ec = os.path.join("Solutions", "arm64ec")

    if os.path.isdir(os.path.join(project_path, arm64ec)):
        return arm64ec
...
def generate_solution(working_directory, cmakelists_dir, config, open_gui):
    ...
    if config.get('cmake_generator_platform'):
        cmake_command.append('-A{}'.format(config['cmake_generator_platform']))
