...
def generate_solution(project_file, code_directory, engine_root_directory):
    ...
    generate_csharp_solution(project_name, project_file,
                             code_directory, engine_root_directory)

    # Now generate the CMakeLists.txt for C++ development
    generate_cpp_cmakelists(project_name, project_file,
                            code_directory, engine_root_directory)


def generate_csharp_solution(
        project_name, project_file, code_directory, engine_root_directory):
    solution_file_template = Template(
...
EndGlobal""")
    ...
    for entry in os.scandir(code_directory):
        if os.path.isdir(entry.path) and not os.path.basename(
                entry.path).startswith("."):
            csproject_guid = uuid.uuid4()

            if generate_csharp_project(
                    project_file, code_directory, os.path.basename(entry.path),
                    engine_root_directory, csproject_guid):
                project_template = Template(
                    'Project("{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}") = '
                    '"$project_name", "$project_name\$project_name.csproj", '
                    '"{$csproject_guid}"')

                projects += project_template.substitute(
                    {'project_name': os.path.basename(entry.path),
                     'csproject_guid': csproject_guid})
                projects += "\nEndProject\n"

                configuration_template = Template(
                    """{$csproject_guid}.Debug|Any CPU.ActiveCfg = Debug|Any CPU
        ...
        {$csproject_guid}.Release|Any CPU.Build.0 = Release|Any CPU""")
                ...
                project_configurations += configuration_template.substitute(
                    {'csproject_guid': csproject_guid})

    if len(projects) > 0:
        solution_file_contents = solution_file_template.substitute(
            {'projects': projects,
             'project_configurations': project_configurations})
        solution_file_path = os.path.join(
            code_directory, project_name + ".sln")
...
def generate_csharp_project(
        project_file, code_directory, cs_source_directory,
        engine_root_directory, csproject_guid):
    cs_project_file_template = Template(
...
""")
    ...
    for filename in glob.iglob(csproj_directory + "/**/*.cs", recursive=True):
        # Ignore the obj directory generated by building
        if not os.path.relpath(
                os.path.abspath(os.path.join(
                    os.path.dirname(filename), os.pardir)),
                csproj_directory) == 'obj':
            includes += '<Compile Include="' + \
                os.path.relpath(filename, csproj_directory) + '" />\n    '

    if len(includes) == 0:
        return False

    engine_bin_directory = os.path.join(
        engine_root_directory, 'bin', 'win_x64')
...
''').substitute({'engine_bin_directory': engine_bin_directory})
    ...
    output_path = os.path.abspath(
        os.path.join(code_directory, os.pardir, "bin"))

    cs_project_file_contents = cs_project_file_template.substitute(
        {'csproject_guid': csproject_guid, 'project_name': cs_source_directory,
         'includes': includes, 'references': references,
         'output_path': output_path,
         'engine_bin_directory': engine_bin_directory,
         'project_file': project_file})

    cs_project_file = open(os.path.join(
        csproj_directory, cs_source_directory) + ".csproj", 'w')
...
if (WIN32)
    set_visual_studio_debugger_command( $${THIS_PROJECT} "$${OUTPUT_DIRECTORY}/GameLauncher.exe" "-project \\"$${PROJECT_DIR}/$projectfile\\"" )
endif()\n'''
    ...
    if os.path.exists(cmakelists_path):
        ...
        current_index = 0
        while True:
            current_index = existing_cmakelists_contents.find(
                custom_block_prefix, current_index)
            if current_index == -1:
                break

            end_index = existing_cmakelists_contents.find(
                custom_block_postfix, current_index + len(custom_block_prefix))
            if end_index == -1:
                break
            ...
            custom_block_contents = (
                existing_cmakelists_contents[current_index:end_index])

            # Check for use of add_subdirectory
            add_subdirectory_index = 0
            while True:
                add_subdirectory_index = custom_block_contents.find(
                    "add_subdirectory", add_subdirectory_index)
                ...
                add_subdirectory_index = custom_block_contents.find(
                    '(', add_subdirectory_index) + 1
                close_scope_index = custom_block_contents.find(
                    ')', add_subdirectory_index)
                ...
                if first_delimiter_index != -1:
                    standalone_project_name = (
                        standalone_project_name[:first_delimiter_index])

                generate_cpp_cmakelists(
                    standalone_project_name, project_file_name, os.path.join(
                        code_directory, standalone_directory),
                    engine_root_directory, False)
        ...
        custom_contents += '\n' + custom_block_prefix + \
            '\n# Make any custom changes here, modifications outside of the ' \
            'block will be discarded on regeneration.\n' + custom_block_postfix
    ...
    startup_project, launcher_projects = get_startup_and_launcher_projects(
        project_name, project_file_name, engine_root_directory)

    output_path = os.path.abspath(os.path.join(
        code_directory, os.pardir, "bin"))
    ...
    cmakelists_contents = cmakelists_template.substitute({
        ...
        'project_path': os.path.abspath(
            os.path.dirname(project_file)).replace('\\', '/'),
...
def get_startup_and_launcher_projects(
        project_name, project_file_name, engine_root_directory):
    platform_file_path = os.path.join(
        engine_root_directory, "Code", "CryEngine", "CryCommon", "CryCore",
        "Platform", "platform.h")
...
if(OPTION_ENGINE)
    if(NOT EXISTS "$${CRYENGINE_DIR}/Code/Sandbox/EditorQt")
        ...
        if (WIN32)
            set_visual_studio_debugger_command(Editor "$${OUTPUT_DIRECTORY}/Sandbox.exe" "-project \\"$${PROJECT_DIR}/$project_file_name\\"")
        endif()
    endif()
else()
    ...
    if (WIN32)
        set_visual_studio_debugger_command(GameLauncher "$${OUTPUT_DIRECTORY}/GameLauncher.exe" "-project \\"$${PROJECT_DIR}/$project_file_name\\"")
    endif()
...
def add_cpp_sources(
        directoryname, project_name, code_directory, skip_directories):
    source_count = 0
    sources = ""
    ...
    for entry in os.scandir(directoryname):
        if os.path.isdir(entry.path) and not os.path.basename(entry.path).startswith(".") and not os.path.exists(os.path.join(entry.path, 'CMakeCache.txt')):
            ...
            directory_sources = add_cpp_sources(
                entry.path, project_name, code_directory, [])
